...
void MFRC522::PCD_Init() {
	...
	PCD_WriteRegister(ModeReg, 0x3D);		// Default 0x3F (ron note: probably meant to be Default 0x3D). Set the preset value for the CRC coprocessor for the CalcCRC command to 0x6363 (ISO 14443-3 part 6.2.4)
...
void MFRC522::PCD_SoftPowerDown(){//Note : Only soft power down mode is available through software
 ...
 * Beware: When two PICCs are in the field at the same time I often get STATUS_TIMEOUT - probably due to bad antenna design.
 * 
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
